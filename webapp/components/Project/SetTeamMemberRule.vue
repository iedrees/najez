<template>
  <div>
        <div class="flex items-center mx-2">
            ...
            <select :disabled="$store.state.user.id == member.user_id" :class="{'opacity-75 cursor-not-allowed':$store.state.user.id == member.user_id}" v-model="member.rule" @change="setRule" class="w-32 border focus:outline-none p-2 h-9 rounded-e bg-white text-cool-gray-500">
