<template>
        <div v-if="project" class="flex items-center">
            <div :class="{'border-green-200 bg-green-100 text-green-500': task.assigned_user != null}" class="flex items-center -ml-px rounded-r-md border px-2 py-3 h-9 text-sm leading-5 h-8">
            ...
            <select  v-model="task.assigned_user_id"  @change="assignUser" class="w-32 border focus:outline-none p-2 h-9 rounded-e bg-white">
                ...
                <option :value="null"> بدون تعيين </option>
                <option disabled> -- </option>
                <option v-if="project" :value="project.user.id">{{project.user.name}}</option>
